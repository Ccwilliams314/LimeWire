...
def init_limewire_styles(root):
    ...
    s.configure("Lime.Horizontal.TProgressbar",
                ...
                darkcolor=T.LIME_DK, thickness=8, borderwidth=0)
    ...
    s.configure("TScrollbar",
                ...
                arrowcolor=T.TEXT_DIM, relief="flat", borderwidth=0, width=10)
    s.map("TScrollbar",
          background=[("active", T.BORDER_L), ("pressed", T.TEXT_DIM)])
    s.configure("Vertical.TScrollbar",
                background=_lerp_color(T.BORDER_L, T.BG, 0.3),
                troughcolor=T.SURFACE_3, width=10)
    s.map("Vertical.TScrollbar",
          background=[("active", T.BORDER_L), ("pressed", T.TEXT_DIM)])

    # Notebook (hidden tabs — we use icon toolbar instead)
    ...
    s.configure("TNotebook.Tab",
                ...
                focuscolor=T.BG)
    s.map("TNotebook.Tab",
          background=[("selected", T.BG), ("!selected", T.BG)],
          foreground=[("selected", T.BG), ("!selected", T.BG)],
          lightcolor=[("selected", T.BG), ("!selected", T.BG)],
          darkcolor=[("selected", T.BG), ("!selected", T.BG)],
          bordercolor=[("selected", T.BG), ("!selected", T.BG)])

    # Treeview
    s.configure("Treeview",
                background=T.INPUT_BG, foreground=T.TEXT,
                fieldbackground=T.INPUT_BG, borderwidth=0,
                font=T.F_BODY, rowheight=28, relief="flat")
